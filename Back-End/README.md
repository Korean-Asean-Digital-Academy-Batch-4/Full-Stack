...
- **Templat komponen nilai (T1-T3, U1-U3, UTS, UAS) sudah di-seed lewat `schema.sql`.**
  ...
  bukan delapan atau total bobot bukan 100%.
...
Endpoint operasional administrator juga menyediakan ringkasan dashboard,
database presensi/nilai/rapor, dan status keamanan database. Jalankan
`npm run audit:db && npm run smoke` sebelum demo atau deployment.
