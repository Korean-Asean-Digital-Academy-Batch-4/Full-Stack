...
async function getMyAttendance(req, res) {
  ...
  const { rows } = await pool.query(
    `SELECT sub.name AS subject_name, sub.grade_level,
            COUNT(*) FILTER (WHERE ar.status = 'Hadir') AS hadir_count,
            COUNT(*) FILTER (WHERE ar.status = 'Izin') AS izin_count,
            COUNT(*) FILTER (WHERE ar.status = 'Sakit') AS sakit_count,
            COUNT(*) FILTER (WHERE ar.status = 'Alpa') AS alpa_count,
  ...
  const withPercent = rows.map((r) => ({
    ...
    present: Number(r.hadir_count),
    permitted: Number(r.izin_count),
    sick: Number(r.sakit_count),
    absent: Number(r.alpa_count),
    presentPercent: Number(r.total_sessions) > 0
      ? Math.round((Number(r.hadir_count) / Number(r.total_sessions)) * 100)
      : null,
  }));
