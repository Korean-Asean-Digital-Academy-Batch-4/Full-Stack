import { AlertCircle, CalendarDays, LoaderCircle, Pencil, RefreshCw, Trash2 } from "lucide-react";
import { useEffect, useState } from "react";
import Button from "../../components/ui/Button";
import Modal from "../../components/ui/Modal";
import ConfirmDialog from "../../components/ui/ConfirmDialog";
import Toast from "../../components/ui/Toast";
import { deleteAdminAttendanceSession, getAdminAttendance, getClasses, updateAdminAttendance } from "../../services/adminService";
...
export default function StudentAttendanceDatabasePage() {
  ...
  const removeSession = async () => {
    ...
    try {
      await deleteAdminAttendanceSession(deleting.session_id);
      setRecords((items) => items.filter((item) => item.session_id !== deleting.session_id));
      setDeleting(null); setDeleteState("idle");
      setToast({ type: "success", message: "Sesi presensi berhasil dihapus." });
    } catch (requestError) { setDeleteError(requestError.message || "Sesi presensi gagal dihapus."); setDeleteState("error"); }
  };
