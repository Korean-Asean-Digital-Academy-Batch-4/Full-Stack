...
export default function SubjectScoreCard({ subject }) {
  ...
  const score = Number.isFinite(subject.score) ? Math.min(100, Math.max(0, subject.score)) : null;
  ...
  return (
    <article className="rounded-[14px] border border-[#D7DCE8] bg-white p-5 shadow-[0_1px_3px_rgba(30,42,75,0.03)]">
      ...
      <div className="mt-2 h-2 overflow-hidden rounded-full bg-[#EEF1F6]" role="progressbar" aria-label={`Nilai ${subject.name}`} aria-valuemin="0" aria-valuemax="100" aria-valuenow={score ?? 0}>
        <div className={`h-full rounded-full ${styles.progress}`} style={{ width: `${score ?? 0}%` }} />
      </div>
      <p className="mt-2 text-right text-sm text-[#545968]">
        <span className="text-lg font-semibold text-[#20232D]">{score == null ? "–" : score}</span> /100
      </p>
    </article>
  );
}
